...
(function () {
  ...
  function injectAuthUI() {
    ...
    loginModal.innerHTML = `
      <div class="auth-modal-box" id="authModalBox">
        ...
        <p class="auth-note">🔒 Your data is protected.</p>
      </div>
    `;
  ...
  function updateProfileMenu(user, data) {
    ...
    if (balanceEl) {
      const usdVal = Number(data?.credit || 0);
      const bdtVal = Math.round(usdVal * 125);
      balanceEl.innerHTML = `<span style="color:#00ff88;">$${usdVal.toLocaleString('en-US', {maximumFractionDigits:2})}</span> <span style="color:#ffa500;font-size:0.8rem;">/ ৳${bdtVal.toLocaleString('en-BD')}</span>`;
    }
  ...
  function iconTxHistory() {
    return `
      <svg class="profile-btn-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v0a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2v0Z" stroke="currentColor" stroke-width="1.7"/>
  ...
  function createProfileMenu(auth) {
    ...
    menu.innerHTML = `
      ...
      <div class="profile-balance"><span>Balance</span><strong id="profileMenuBalance">$0</strong></div>
      <button class="nav-user-menu-btn" id="profileSettingsBtn" type="button">${iconSettings()} <span>Settings</span></button>
      <button class="nav-user-menu-btn" id="profileTxHistoryBtn" type="button">${iconTxHistory()} <span>Transaction History</span></button>
      <a class="nav-user-menu-btn primary" href="add-credit.html">${iconCredit()} <span>Add Credit</span></a>
    ...
    const txHistoryBtn = menu.querySelector('#profileTxHistoryBtn');
    if (txHistoryBtn) txHistoryBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      menu.classList.remove('open');
      ...
      <div class="profile-settings-box">
        <div class="settings-head">
          <div>
            ...
            <p>Update your name, profile picture, and account password securely.</p>
          </div>
          <button class="settings-close" id="profileSettingsClose" type="button" aria-label="Close settings">&times;</button>
        </div>

        <div class="settings-section">
          <div class="settings-section-title">${iconSettings()} <span>Account Information</span></div>
          <label class="settings-label" for="settingsName">Full Name</label>
          <input class="settings-input" id="settingsName" type="text" placeholder="Your name" />

          <label class="settings-label" for="settingsEmail">Email</label>
          <input class="settings-input" id="settingsEmail" type="email" readonly />

          <label class="settings-label" for="settingsPhotoInput">Profile Picture</label>
          <input class="settings-input" id="settingsPhotoInput" type="file" accept="image/*" />

          <div class="settings-actions">
            <button class="settings-btn" id="settingsSaveProfileBtn" type="button">${iconSave()} <span>Save Profile</span></button>
          </div>
        </div>

        <div class="settings-section">
          <div class="settings-section-title">${iconPassword()} <span>Password</span></div>
          <label class="settings-label" for="settingsCurrentPassword">Current Password</label>
          <input class="settings-input" id="settingsCurrentPassword" type="password" placeholder="Current password" />

          <label class="settings-label" for="settingsNewPassword">New Password</label>
          <input class="settings-input" id="settingsNewPassword" type="password" placeholder="Minimum 6 characters" />

          <div class="settings-actions">
            <button class="settings-btn" id="settingsChangePasswordBtn" type="button">${iconPassword()} <span>Change Password</span></button>
            <button class="settings-btn secondary" id="settingsResetPasswordBtn" type="button">${iconPassword()} <span>Send Reset Link</span></button>
          </div>
        </div>

        <div class="settings-section">
          <div class="settings-section-title">${iconSettings()} <span>Language / ভাষা</span></div>
          <label class="settings-label">Interface Language</label>
          <div class="radio-group" style="flex-direction:column;gap:10px;margin-top:6px;">
            <label class="radio-option" style="display:flex;align-items:center;gap:10px;cursor:pointer;color:#c9d4e5;">
              <input type="radio" name="siteLanguage" value="en" id="langEn" style="accent-color:var(--accent);" /> English
            </label>
            <label class="radio-option" style="display:flex;align-items:center;gap:10px;cursor:pointer;color:#c9d4e5;">
              <input type="radio" name="siteLanguage" value="bn" id="langBn" style="accent-color:var(--accent);" /> বাংলা (Bangla)
            </label>
          </div>
          <div class="settings-actions" style="margin-top:12px;">
            <button class="settings-btn" id="saveLangBtn" type="button">${iconSave()} <span>Save Language</span></button>
          </div>
        </div>

        <div id="profileSettingsMessage" class="settings-message"></div>
      </div>
    `;
  ...
  async function saveProfileSettings() {
    ...
    try {
      ...
      setSettingsMessage('success', 'Profile updated successfully! Refreshing...');
      // Reload page after short delay so navbar displays updated name
  ...
  function openTransactionHistory() {
        ...
        <div class="profile-settings-box">
          <div class="settings-head">
            <div>
              ...
              <p>আপনার সব payment request এবং service order এর ইতিহাস।</p>
            </div>
            <button class="settings-close" id="txHistoryClose" type="button" aria-label="Close">&times;</button>
          </div>
          <div id="txHistoryContent" style="margin-top:8px;">
            <div style="text-align:center;padding:24px;color:#7a8ca8;font-size:0.85rem;">Loading...</div>
          </div>
        </div>
      `;
      document.body.appendChild(modal);

      const closeBtn = modal.querySelector('#txHistoryClose');
    ...
    modal.classList.add('open');
    document.body.style.overflow = 'hidden';
    loadTransactionHistory(modal.querySelector('#txHistoryContent'));
  }

  async function loadTransactionHistory(container) {
    if (!container || !dbApi || !currentUser) return;
    container.innerHTML = `<div style="text-align:center;padding:24px;color:#7a8ca8;font-size:0.85rem;">Loading...</div>`;

    try {
      const { db, collection, query, where, orderBy, getDocs, limit } = dbApi;

      // Load topups
      const tq = query(collection(db, 'topups'), where('userId', '==', currentUser.uid));
      const oq = query(collection(db, 'serviceOrders'), where('userId', '==', currentUser.uid));

      const [topupSnap, orderSnap] = await Promise.all([getDocs(tq), getDocs(oq)]);

      const items = [];

      topupSnap.forEach(d => {
        const data = d.data();
        items.push({ type: 'topup', id: d.id, ...data, _ts: data.createdAt?.seconds || 0 });
      });

      orderSnap.forEach(d => {
        const data = d.data();
        items.push({ type: 'order', id: d.id, ...data, _ts: data.createdAt?.seconds || 0 });
      });

      items.sort((a, b) => b._ts - a._ts);

      if (items.length === 0) {
        container.innerHTML = `<div style="text-align:center;padding:32px;color:#7a8ca8;font-size:0.85rem;">No transactions found.</div>`;
        return;
      }
      ...
      function statusBadge(status) {
        const colors = {
          ...
          approved: '#00ff88',
          declined: '#ff5050',
          paid_credit_pending_review: '#00c8ff',
          paid_instant_pending_review: '#a78bfa',
          accepted: '#00ff88',
          completed: '#00ff88'
        };
        const labels = {
          ...
          approved: 'Approved',
          declined: 'Declined',
          paid_credit_pending_review: 'Credit Paid',
          paid_instant_pending_review: 'Instant Paid',
          accepted: 'Accepted',
          completed: 'Completed'
        };
        const color = colors[status] || '#7a8ca8';
        const label = labels[status] || status;
        return `<span style="background:${color}18;color:${color};border:1px solid ${color}44;border-radius:8px;padding:2px 9px;font-size:0.72rem;font-weight:700;white-space:nowrap;">${label}</span>`;
      }

      let html = `<div style="display:flex;flex-direction:column;gap:10px;">`;

      items.forEach(item => {
        const dateStr = item._ts ? new Date(item._ts * 1000).toLocaleDateString('en-BD', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
        const usdAmt = Number(item.amountUsd || item.amountUSD || 0);
        const bdtAmt = Number(item.amountBdt || item.amountBDT || Math.round(usdAmt * 125) || 0);

        if (item.type === 'topup') {
          html += `
            <div style="background:rgba(255,255,255,0.035);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:12px 14px;">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px;">
                <div style="display:flex;align-items:center;gap:8px;">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00c8ff" stroke-width="1.7" style="flex-shrink:0"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z"/></svg>
                  <span style="color:#e8edf5;font-weight:700;font-size:0.86rem;">Credit Top-up${item.serviceName ? ' — ' + item.serviceName : ''}</span>
                </div>
                ${statusBadge(item.status)}
              </div>
              <div style="display:flex;gap:12px;flex-wrap:wrap;color:#7a8ca8;font-size:0.75rem;">
                <span style="color:#00ff88;font-weight:700;">$${usdAmt.toFixed(2)}</span>
                <span style="color:#ffa500;">৳${bdtAmt}</span>
                <span>${item.method || '—'}</span>
                <span>TX: ${item.transactionId || '—'}</span>
                <span style="margin-left:auto;">${dateStr}</span>
              </div>
            </div>`;
        } else {
          html += `
            <div style="background:rgba(255,255,255,0.035);border:1px solid rgba(0,200,255,0.1);border-radius:14px;padding:12px 14px;">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px;">
                <div style="display:flex;align-items:center;gap:8px;">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" stroke-width="1.7" style="flex-shrink:0"><path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75"/></svg>
                  <span style="color:#e8edf5;font-weight:700;font-size:0.86rem;">${item.serviceName || 'Service Order'}</span>
                </div>
                ${statusBadge(item.status)}
              </div>
              <div style="display:flex;gap:12px;flex-wrap:wrap;color:#7a8ca8;font-size:0.75rem;">
                <span style="color:#00ff88;font-weight:700;">$${usdAmt.toFixed(2)}</span>
                <span style="color:#ffa500;">৳${bdtAmt}</span>
                <span>${item.paymentMethod || 'credit'}</span>
                <span style="margin-left:auto;">${dateStr}</span>
              </div>
            </div>`;
        }
      ...
      container.innerHTML = `<div style="text-align:center;padding:24px;color:#ff8080;font-size:0.85rem;">Failed to load transactions. Please try again.</div>`;
    }
  }
