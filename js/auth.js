...
(function () {
  ...
  function bdtFromUsd(value) {
    return `${Math.round(Number(value || 0) * 125).toLocaleString('en-BD')} BDT`;
  }

  async function loadSettingsTransactionHistory() {
    const box = document.getElementById('settingsTransactionHistory');
    if (!box) return;

    if (!currentUser || !dbApi) {
      box.innerHTML = '<div class="settings-history-empty">Login information is not ready. Please refresh the page.</div>';
      return;
    }

    box.innerHTML = '<div class="settings-history-empty">Loading transaction history...</div>';

    try {
      const { db, collection, query, where, getDocs } = dbApi;
      const items = [];

      const topupSnap = await getDocs(query(collection(db, 'topups'), where('userId', '==', currentUser.uid)));
      topupSnap.forEach((docSnap) => {
        const data = docSnap.data() || {};
        items.push({
          type: (data.purpose || 'credit') === 'service' ? 'Instant Service Payment' : 'Credit Top-up',
          title: data.serviceName || 'Wallet Credit',
          amountUsd: Number(data.amountUsd || data.amountUSD || data.amount || 0),
          amountBdt: Number(data.amountBdt || data.amountBDT || Math.round(Number(data.amountUsd || data.amountUSD || data.amount || 0) * 125)),
          status: data.status || 'pending',
          method: data.method || 'Manual',
          transactionId: data.transactionId || '',
          createdAt: data.createdAt || data.approvedAt || data.declinedAt
        });
      });

      const orderSnap = await getDocs(query(collection(db, 'serviceOrders'), where('userId', '==', currentUser.uid)));
      orderSnap.forEach((docSnap) => {
        ...
        items.push({
          ...
          amountBdt: Number(data.amountBdt || data.amountBDT || Math.round(Number(data.amountUsd || data.amountUSD || 0) * 125)),
          status: data.status || 'pending',
      ...
      if (!items.length) {
        box.innerHTML = '<div class="settings-history-empty">No transaction history found yet.</div>';
        return;
      }

      box.innerHTML = items.slice(0, 15).map((item) => `
        <div class="settings-history-item">
          <div class="settings-history-top">
            <div class="settings-history-title">${escapeHtml(item.type)} — ${escapeHtml(item.title)}</div>
            <div class="settings-history-amount">${dollar(item.amountUsd)}</div>
          </div>
          <div class="settings-history-meta">${escapeHtml(`${Number(item.amountBdt || 0).toLocaleString('en-BD', { maximumFractionDigits: 0 })} BDT`)} • ${escapeHtml(item.method)}</div>
          ${item.transactionId ? `<div class="settings-history-meta">Transaction ID: ${escapeHtml(item.transactionId)}</div>` : ''}
          <div class="settings-history-meta">${escapeHtml(formatDate(item.createdAt))}</div>
          <span class="settings-history-status">${escapeHtml(item.status)}</span>
        </div>
      `).join('');
    } catch (err) {
      box.innerHTML = `<div class="settings-history-empty">Unable to load history: ${escapeHtml(err.message || 'Unknown error')}</div>`;
    }
  }
  ...
  function createProfileSettingsModal() {
    ...
    modal.innerHTML = `
      <div class="profile-settings-box">
        ...
        <div class="settings-section">
          <div class="settings-section-title">${iconHistory()} <span>Transaction History</span></div>
          <div class="settings-actions" style="margin-top:0;">
            <button class="settings-btn secondary" id="settingsRefreshHistoryBtn" type="button">${iconHistory()} <span>Refresh History</span></button>
          </div>
          <div id="settingsTransactionHistory" class="settings-history-list">
            <div class="settings-history-empty">History will load when settings open.</div>
          </div>
        </div>
    ...
    const refreshHistoryBtn = modal.querySelector('#settingsRefreshHistoryBtn');
    ...
    if (refreshHistoryBtn) refreshHistoryBtn.addEventListener('click', loadSettingsTransactionHistory);
  ...
  function openProfileSettings() {
    ...
    fillSettingsForm();
    loadSettingsTransactionHistory();
