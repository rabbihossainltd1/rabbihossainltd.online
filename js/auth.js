  ...
  function createProfileMenu(auth) {
    ...
    menu.innerHTML = `
      ...
      <div class="profile-upload">
        <button class="nav-user-menu-btn" id="profileUploadBtn" type="button">${professionalImageIcon()} <span>Set Profile Picture</span></button>
        <input id="profilePhotoInput" type="file" accept="image/*" style="display:none;" />
        <div class="profile-mini-note">Max compressed automatically before saving.</div>
      </div>
    ...
    const uploadBtn = menu.querySelector('#profileUploadBtn');
    const input = menu.querySelector('#profilePhotoInput');
    const settingsBtn = menu.querySelector('#profileSettingsBtn');
    const signOutBtn = menu.querySelector('#navSignOutBtn');

    if (uploadBtn && input) uploadBtn.addEventListener('click', (e) => { e.stopPropagation(); input.click(); });
