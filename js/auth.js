/* ============================================================
   Rabbi Portfolio — Auth System v2.0
...
(function () {
  ...
  function injectAuthCSS() {
    ...
    style.textContent = `
      ...
      @media(max-width:700px){ .nav-user-menu{ right:-8px; min-width:260px; } }
    `;
  ...
  async function checkAdmin(user) {
    ...
    if (!dbApi || !user) return false;
    try {
      const { db, doc, getDoc } = dbApi;
      const snap = await getDoc(doc(db, 'admins', user.uid));
      currentIsAdmin = snap.exists();
      const adminBtn = document.getElementById('navAdminBtn');
      if (adminBtn) adminBtn.hidden = !currentIsAdmin;
  ...
  function createProfileMenu(auth) {
    ...
    menu.innerHTML = `
      ...
      <div class="profile-upload">
        <button class="nav-user-menu-btn" id="profileUploadBtn" type="button">🖼️ Set Profile Picture</button>
      ...
      <a class="nav-user-menu-btn primary" href="add-credit.html">💳 Add Credit</a>
      <a class="nav-user-menu-btn" id="navAdminBtn" href="admin.html" hidden>🛡️ Admin Panel</a>
      <button class="nav-user-menu-btn danger" id="navSignOutBtn" type="button">🚪 Sign Out</button>
    `;
  ...
  async function handleProfilePhotoUpload(event) {
    ...
    try {
      ...
      if (uploadBtn) { uploadBtn.disabled = false; uploadBtn.textContent = oldText || '🖼️ Set Profile Picture'; }
    }
  }
