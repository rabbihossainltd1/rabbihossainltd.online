...
async function ensureUserDoc(user) {
  ...
  if (!snap.exists()) {
    ...
    // Only update email if missing — NEVER overwrite name or photoURL
...
window.buyServiceWithCredit = async function (servicePayload) {
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const newCredit = Number((currentCredit - amountUsd).toFixed(2));
      const amountBdt = toBdt(amountUsd);
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if ((topup.purpose || "credit") === "service") {
        ...
        transaction.update(userRef, {
          credit: increment(Number(topup.amountUsd || topup.amountUSD || 0)),
          updatedAt: serverTimestamp()
        });
      }
    });
