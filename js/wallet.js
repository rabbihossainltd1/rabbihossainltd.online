...
window.buyServiceWithCredit = async function (servicePayload) {
  ...
  const legacyOrderRef = doc(db, "orders", orderRef.id);
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.set(legacyOrderRef, orderPayload);
    });
    ...
    if (error.message && error.message.startsWith("INSUFFICIENT_CREDIT")) {
      ...
      return { ok: false, reason: "insufficient", balance, message: `Insufficient credit. Current balance: ${moneyPair(balance)}.` };
    }
...
window.loadAdminPanel = function () {
  onAuthStateChanged(auth, async (user) => {
    ...
    onSnapshot(orderQuery, (snapshot) => {
      ...
      if (snapshot.empty) {
        orderList.innerHTML = `<div class="empty-state">No service order found.</div>`;
        return;
      }
      ...
      docs.sort((a, b) => {
        const da = a.data() || {};
        const dbb = b.data() || {};
        const pa = String(da.status || "").includes("pending") ? 0 : 1;
        const pb = String(dbb.status || "").includes("pending") ? 0 : 1;
        return pa - pb;
      });
