...
const BACKEND_API_BASE = "https://rabbi-backend.vercel.app";

function backendRoute(path) {
  if (!path) return "/api/index?route=health";
  if (path.startsWith("/api/index")) return path;
  if (path.startsWith("/api/")) {
    return "/api/index?route=" + encodeURIComponent(path.replace("/api/", ""));
  }
  return path;
}
...
function adminActionText(data) {
  ...
  if (status === "accepted" || status === "confirmed" || status === "completed") return "Accepted by admin";
...
window.loadUserDashboard = function () {
  onAuthStateChanged(auth, async (user) => {
    ...
    // Load service orders and payment requests for the My Orders section
    loadDashboardOrders(user);
    loadDashboardTopups(user);
    ...
    docs.forEach((docSnap) => {
      ...
      list.innerHTML += `
        <div class="dashboard-history-card">
          ...
          <p>Payment: ${escapeHtml(data.paymentMethod || "credit")}${getFreeFireUid(data) ? " • UID: " + escapeHtml(getFreeFireUid(data)) : ""}${data.providerOrderId ? " • Provider ID: " + escapeHtml(data.providerOrderId) : ""}</p>
