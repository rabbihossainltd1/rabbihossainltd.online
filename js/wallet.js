    ...
    snapshot.forEach((docSnap) => {
      ...
      list.innerHTML += `
        <div class="request-card">
          <div>
            <strong>${usd(data.amountUsd || data.amount || 0)} / ${bdt(data.amountBdt || toBdt(data.amountUsd || 0))}</strong>
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.update(userRef, {
        credit: currentCredit - amountUsd,
        updatedAt: serverTimestamp()
      });
        ...
        topupList.innerHTML += `
          <div class="admin-card">
            <div class="admin-card-head">
              <h3>${usd(data.amountUsd || data.amount || 0)} / ${bdt(data.amountBdt || toBdt(data.amountUsd || 0))}</h3>
      ...
      snapshot.forEach((docSnap) => {
        ...
        orderList.innerHTML += `
          <div class="admin-card">
            ...
            <p><b>Amount:</b> ${usd(data.amountUsd)} / ${bdt(data.amountBdt)}</p>
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if ((topup.purpose || "credit") === "service") {
        ...
        transaction.set(orderRef, {
          ...
          amountUsd: Number(topup.amountUsd || 0),
          amountBdt: Number(topup.amountBdt || 0),
          rate: topup.rate || USD_TO_BDT,
