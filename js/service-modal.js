...
(function () {
  ...
  const DEFAULT_SERVICE_PRICES = {
    ...
    'Free Fire Diamond Top-up': 2,
    'Free Fire Android Panel (Drip)': 5,
    'Free Fire Android Panel (FF4X)': 5,
    'Free Fire iPhone Panel (iOS)': 5,
    'Free Fire PC Panel': 5
  };
  ...
  const allFields = {
    ...
    ff: document.getElementById('ffFields'),
    ffPanel: document.getElementById('ffPanelFields')
  };

  // Bug 6: Physical card address toggle
  ...
  function showFields(type) {
    ...
    // Show diamond amount group only for diamond service
    const diamondGroup = document.getElementById('ffDiamondAmountGroup');
    if (diamondGroup) {
      diamondGroup.style.display = (type === 'ff') ? 'block' : 'none';
    }
  }
  ...
  function validateBasicDetails() {
    ...
    if (!name || !email) {
      showStatus('⚠ Payment করার আগে Name এবং Email পূরণ করো।', 'error');
      return false;
    }
  ...
  function handleInstantPay() {
    ...
    const userCredit = window.rabbiAuth ? window.rabbiAuth.getCredit() : 0;

    if (userCredit >= amountUsd) {
      // User has enough balance - use credit directly
      handleBuyWithCredit();
      return;
    }

    // Not enough balance - go to add credit page with pre-filled amount
